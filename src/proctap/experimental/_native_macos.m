...
static OSStatus create_process_tap(ProcessTapState* state) {
    @autoreleasepool {
        ...
        // Note: The tap object itself doesn't expose a UID property,
        // so we generate one to reference it in the aggregate device configuration
        CFUUIDRef tap_uuid = CFUUIDCreate(NULL);
        CFStringRef tap_uuid_str = CFUUIDCreateString(NULL, tap_uuid);
        NSString* tap_uuid_str_ns = (__bridge NSString*)tap_uuid_str;
        CFRelease(tap_uuid);
        ...
        NSString* device_uid_str = (__bridge NSString*)device_uid;
        ...
        NSDictionary* device_dict = @{
            ...
            (__bridge NSString*)kAudioAggregateDeviceUIDKey: [NSString stringWithFormat:@"com.proctap.native.%@", [[NSUUID UUID] UUIDString]],
            ...
            (__bridge NSString*)kAudioAggregateDeviceTapAutoStartKey: @YES,  // Critical for low latency!
            (__bridge NSString*)kAudioAggregateDeviceSubDeviceListKey: @[
        ...
        data_size = sizeof(AudioObjectID);  // Reuse existing variable
        status = AudioObjectGetPropertyData(
            kAudioObjectSystemObject,
            &address,
...
static PyObject* start_tap(PyObject* self, PyObject* args) {
    ...
    if (atomic_load(&state->is_running)) {
        Py_RETURN_NONE;  // Already running
    }
    ...
    if (status != noErr) {
        PyErr_Format(PyExc_RuntimeError, "Failed to create IOProc: %d", status);
        return NULL;
    }

    // Start device
    status = AudioDeviceStart(state->device_id, io_proc_callback);
    if (status != noErr) {
        AudioDeviceDestroyIOProcID(state->device_id, state->io_proc_id);
        PyErr_Format(PyExc_RuntimeError, "Failed to start device: %d", status);
        return NULL;
    }
...
static PyObject* stop_tap(PyObject* self, PyObject* args) {
    ...
    if (!atomic_load(&state->is_running)) {
        Py_RETURN_NONE;  // Already stopped
    }

    atomic_store(&state->is_running, false);

    AudioDeviceStop(state->device_id, io_proc_callback);
    if (state->io_proc_id) {
        AudioDeviceDestroyIOProcID(state->device_id, state->io_proc_id);
    }
